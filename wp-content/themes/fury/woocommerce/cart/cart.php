	...
	<table class="table shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
		<thead>
			<tr>
				...
				<th class="product-subtotal text-center"><?php esc_html_e( 'Total', 'fury' ); ?></th>
			</tr>
		</thead>
		<tbody>
			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
						...
						<td class="product-remove text-center">
							<?php
								...
								echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf(
									'<a href="%s" class="remove-from-cart" data-toggle="tooltip" data-original-title="'. esc_html__( 'Remove item', 'fury' ) .'" aria-label="%s" data-product_id="%s" data-product_sku="%s">&times;</a>',
									esc_url( wc_get_cart_remove_url( $cart_item_key ) ),
									__( 'Remove this item', 'fury' ),
									esc_attr( $product_id ),
						...
						<td class="product-name" data-title="<?php esc_attr_e( 'Product', 'fury' ); ?>"><div class="product-item"><?php
						$thumbnail = apply_filters( 'woocommerce_cart_item_thumbnail', $_product->get_image(), $cart_item, $cart_item_key );

						if ( ! $product_permalink ) {
							echo wp_kses_post( $thumbnail );
						} else {
							printf( '<a class="product-thumb" href="%s">%s</a>', esc_url( $product_permalink ), wp_kses_post( $thumbnail ) );
						}
                        ?>
						...
						if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) ) {
							echo wp_kses_post( apply_filters( 'woocommerce_cart_item_backorder_notification', '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'fury' ) . '</p>' ) );
						}
                        ?></div></div></td>
						...
						<td class="product-subtotal text-center text-lg text-medium" data-title="<?php esc_attr_e( 'Total', 'fury' ); ?>">
                            <?php
