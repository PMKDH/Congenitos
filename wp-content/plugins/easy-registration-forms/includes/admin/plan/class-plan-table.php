...
class ERForms_Plan_Table extends ERForms_List_Table {
		  ...
		  switch ( $column_name ) {
			  case 'id':
				  ...
				  break;
                          ...
                          case 'type':
				  $value =  $plan['type']=='fixed' ? __('Fixed','erforms') : __('User Defined','erforms'); break;
                              
                          case 'price':
				  $value = $plan['price']; break;
	...
	public function process_bulk_actions() {
		...
		if ( 'delete' === $this->current_action() ) {
			...
			if (
				wp_verify_nonce( $_GET['_wpnonce'], 'bulk-plans' ) ||
				wp_verify_nonce( $_GET['_wpnonce'], 'erforms_delete_plan_nonce' )
			) {
				foreach ( $ids as $id ) {
