...
class ERForms_Attachment_Cards extends ERForms_List_Cards {
	...
	public function prepare_items() {
		...
		$out = '<div class="erf-card-content erf-pad10">';
                ...
                if (!empty($cover_image_attr)) {
                    $out .= '<div class="erf-attachment"><a href="?page=erforms-dashboard&form_id='.$submission['form_id'].'&submission_id='.$submission['id'].'&tab=submission_attachments">'
                            .'<img src="'.$cover_image_attr[0].'" width="'.$cover_image_attr[1].'" height="'.$cover_image_attr[2].'" />'.
                        '</a></div>';
                }
                else
                {
                    $out .= '<div class="erf-attachment"><a href="?page=erforms-dashboard&form_id='.$submission['form_id'].'&submission_id='.$submission['id'].'&tab=submission_attachments">'
                            . '<img src="' .ERFORMS_PLUGIN_URL .'/assets/admin/images/attachment-file.png"></a></div>';
                }
                ...
                $out .= '<div class="erf-attachment-submission-view"><a target="__blank" href="?page=erforms-submission&view=submission&submission_id='.$submission['id'].'">'.__('Submission','erforms').'</a></div>';
