...
class CheckoutView {
	...
	public static function renderBookingDetailsInner( $booking, $details ){
		...
		<div class="mphb-reserve-rooms-details">
			<?php
				foreach ( $booking->getReservedRooms() as $number => $reservedRoom ) {
			...
			?>
					<div class="mphb-room-details">
						<input type="hidden" name="mphb_room_details[<?php echo esc_attr( $number ); ?>][room_type_id]" value="<?php echo esc_attr( $roomType->getOriginalId() ) ?>" />
						<input type="hidden" name="mphb_room_details[<?php echo esc_attr( $number ); ?>][room_id]" value="<?php echo esc_attr( $reservedRoom->getRoomId() ) ?>" />

						<?php do_action( 'mphb_cb_checkout_room_details', $roomType, $number, $booking, $details ); ?>
					</div>
			<?php } ?>
