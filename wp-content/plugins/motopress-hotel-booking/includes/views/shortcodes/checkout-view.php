...
class CheckoutView {
	...
	public static function renderGuestsChooser( $roomType, $roomKey, $booking ){
		...
		if ( count( $reservedRooms ) == 1 && !isset( $_REQUEST['mphb_skip_single_room_preset'] ) ) {
			if ( MPHB()->settings()->main()->isDirectBooking() && isset( $_REQUEST['mphb_is_direct_booking']) ) {
		...
		<p class="mphb-adults-chooser">
			...
			<select name="<?php echo $namePrefix . '[adults]'; ?>" id="<?php echo $idPrefix . '-adults'; ?>" class="mphb_sc_checkout-guests-chooser mphb_checkout-guests-chooser" required="required">
		...
		<?php if ( $roomType->getChildrenCapacity() > 0 ) { ?>
			<p class="mphb-children-chooser">
				...
				<select name="<?php echo $namePrefix . '[children]' ?>" id="<?php echo $idPrefix . '-children'; ?>" class="mphb_sc_checkout-guests-chooser mphb_checkout-guests-chooser" required="required">
					<option value=""><?php _e( '— Select —', 'motopress-hotel-booking' ); ?></option>
					<?php
