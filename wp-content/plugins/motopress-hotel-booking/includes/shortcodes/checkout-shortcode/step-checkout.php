...
class StepCheckout extends Step {
	 ...
	 * @var array
	 */
	protected $roomTypesDetails = array();
	...
	public function addInitActions(){
		...
		if ( MPHB()->settings()->main()->getConfirmationMode() === 'payment' ) {
			...
			if ( count( $gateways ) == 1 && apply_filters( 'mphb_checkout_single_gateway_hide_billing_details', false, reset( $gateways ) ) ) {
...
//				'mphb_language'	 => 'original'
			...
			$this->roomTypesDetails[$roomType->getOriginalId()] = array(
		...
		// If there is only one reserved room, then update it's capacity using
		// search parameters
		if ( count( $this->reservedRooms ) == 1 ) {
			$reservedRoom	 = reset( $this->reservedRooms );
			...
			if ( $roomType ) {
				...
				$reservedRoomAtts = array(
					'rate_id'	 => $reservedRoom->getRateId(),
					'adults'	 => $maxAdults,
					'children'	 => $maxChildren
				);

				$searchParameters = MPHB()->searchParametersStorage()->get();
				if ( isset( $searchParameters['mphb_adults'] ) && $searchParameters['mphb_adults'] !== '' && $searchParameters['mphb_adults'] <= $maxAdults ) {
					$reservedRoomAtts['adults'] = $searchParameters['mphb_adults'];
				}
				if ( isset( $searchParameters['mphb_children'] ) && $searchParameters['mphb_children'] !== '' && $searchParameters['mphb_children'] <= $maxChildren ) {
					$reservedRoomAtts['children'] = $searchParameters['mphb_children'];
				}

				$this->reservedRooms = array( Entities\ReservedRoom::create( $reservedRoomAtts ) );
			}
		}
	...
	public function render(){
		...
		\MPHB\Views\Shortcodes\CheckoutView::renderCheckoutForm( $this->booking, $this->roomTypesDetails );
