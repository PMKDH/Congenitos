...
//			Configuration::sslVersion( 6 );
	...
	private function generateAdminDescription(){
		$description = sprintf( __( 'Webhooks Destination URL: %s' ), '<code>' . esc_url( $this->webhookListener->getNotifyUrl() ) . '</code>' );
	...
	public function registerOptionsFields( &$subTab ){
		...
		} else if ( !MPHB()->isSiteSSL() && (!MPHB()->settings()->payment()->isForceCheckoutSSL() && !class_exists( 'WordPressHTTPS' ) ) ) {
		...
		$groupFields = array(
			...
			Fields\FieldFactory::create( "mphb_payment_gateway_{$this->id}_private_key", array(
				'type'			=> 'text',
				'label'			=> __( 'Private Key', 'motopress-hotel-booking' ),
				'default'		=> $this->getDefaultOption( 'private_key' )
			) ),
			Fields\FieldFactory::create( "mphb_payment_gateway_{$this->id}_account_id", array(
				'type'			=> 'text',
