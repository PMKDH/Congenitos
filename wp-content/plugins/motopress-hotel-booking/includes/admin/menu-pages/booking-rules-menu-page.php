...
use \MPHB\BookingRules\Reservation\RulesBuilder;
use \MPHB\BookingRules\Reservation\SeasonPriorities;
use \MPHB\BookingRules\Reservation\SeasonRulesBuilder;
...
class BookingRulesMenuPage extends AbstractMenuPage {
	...
	private function processReservationRules(){
		$typeRulesBuilder = new RulesBuilder();
		$seasonRulesBuilder = new SeasonRulesBuilder();
		$seasonPriorities = new SeasonPriorities();
		...
		foreach( $postFields as $postField ) {
			...
			$typeRulesBuilder->addRules( $postValues );
			$seasonRulesBuilder->addRules( $postValues );
			$seasonPriorities->addRules( $postValues );
		}

		// Generate new reservation rules
		$reservationRules = $typeRulesBuilder->build();
		update_option( 'mphb_booking_rules_reservation', $reservationRules, 'no' );

		// Generate season reservation rules
		$seasonRules = $seasonRulesBuilder->build();
		update_option( 'mphb_booking_rules_seasons', $seasonRules, 'no' );

		$priorities = $seasonPriorities->calcPriorities();
		update_option( 'mphb_booking_rules_season_priorities', $priorities, 'no' );
	}
