...
class CheckoutStep extends Step {
	...
	/** @var array [%Room type ID% => [\MPHB\Entities\Rate[] allowed_rates]] */
	...
	public function setup(){
		...
		add_action( 'mphb_cb_checkout_form', array( '\MPHB\Views\CreateBooking\CheckoutView', 'renderBookingDetails' ), 10, 2 );
			...
			add_action( 'mphb_cb_checkout_booking_details', array( '\MPHB\Views\CreateBooking\CheckoutView', 'renderBookingDetailsInner' ), 30, 2 );
				add_action( 'mphb_cb_checkout_room_details', array( '\MPHB\Views\Shortcodes\CheckoutView', 'renderRoomTypeTitle' ), 10, 2 );
				add_action( 'mphb_cb_checkout_room_details', array( '\MPHB\Views\Shortcodes\CheckoutView', 'renderGuestsChooser' ), 20, 3 );
				add_action( 'mphb_cb_checkout_room_details', array( '\MPHB\Views\Shortcodes\CheckoutView', 'renderRateChooser' ), 30, 4 );
				add_action( 'mphb_cb_checkout_room_details', array( '\MPHB\Views\Shortcodes\CheckoutView', 'renderServiceChooser' ), 40, 2 );
		...
		foreach ( $this->rooms as $room ) {
			...
			$this->details[$roomTypeId] = array(
				'allowed_rates' => $room['allowed_rates']
			);
		}
	}
	...
	protected function parseRooms( $input ){
		...
		foreach ( $request as $roomTypeId => $roomIds ) {
			...
			if ( !MPHB()->getRoomPersistence()->isRoomsFree( $this->checkInDate, $this->checkOutDate, $roomIds, $roomTypeId ) ) {
