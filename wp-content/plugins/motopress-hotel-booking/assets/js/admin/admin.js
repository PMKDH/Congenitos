		...
		$.each( ctrls, function() {
			...
			switch ( type ) {
				...
				case 'rules-list':
					...
					break;
				case 'variable-price':
					ctrl = new MPHB.VariablePriceCtrl( $( this ) );
					break;
			}
 ...
 * @requires ./ctrl.js
 */
MPHB.VariablePriceCtrl = MPHB.Ctrl.extend( {}, {
	init: function( el, args ) {
		this._super( el, args );

		var variablePrices = this.element.find( '.mphb-ctrl-variable-prices' );

		this.element.find( '.mphb-enable-variable-pricing' ).on( 'change', function( e ) {
			variablePrices.toggleClass( 'mphb-hide' );
		} );
	}
} );
	new MPHB.Plugin();
