...
function mphb_tmpl_the_room_type_default_price( $id = null ){
	...
	$nights	 = MPHB()->settings()->bookingRules()->getMinDaysStay( $roomType->getId() );
...
function mphb_tmpl_has_room_type_default_price( $id = null ){
	$roomType = $id ? MPHB()->getRoomTypeRepository()->findById( $id ) : MPHB()->getCurrentRoomType();
	return $roomType->getDefaultPrice() > 0;
}
...
function mphb_tmpl_the_room_type_price_for_dates( \DateTime $checkInDate, \DateTime $checkOutDate ){
	$price = MPHB()->getCurrentRoomType()->getDefaultPriceForDates( $checkInDate, $checkOutDate );
...
function mphb_tmpl_the_room_reservation_form( $roomType = null ){
	...
	?>
	<form method="<?php echo $formMethod; ?>" action="<?php echo esc_url( $actionUrl ); ?>" class="mphb-booking-form" id="booking-form-<?php echo $typeId; ?>">
		...
		?>
		<input type="hidden" name="mphb_room_type_id" value="<?php echo $typeId; ?>" />
		<p class="mphb-check-in-date-wrapper">
			...
			<br />
			<input id="mphb_check_in_date-<?php echo $uniqueSuffix; ?>" type="text" class="mphb-datepick" value="<?php echo esc_attr( $checkInDateFormatted ); ?>" required="required" autocomplete="off"/>
			<input id="mphb_check_in_date-<?php echo $uniqueSuffix; ?>-hidden" type="hidden" name="mphb_check_in_date" value="<?php echo esc_attr( $checkInDate ); ?>" />
		</p>
		<p class="mphb-check-out-date-wrapper">
			...
			<br />
			<input id="mphb_check_out_date-<?php echo $uniqueSuffix; ?>" type="text" class="mphb-datepick" value="<?php echo esc_attr( $checkOutDateFormatted ); ?>" required="required" autocomplete="off"/>
			<input id="mphb_check_out_date-<?php echo $uniqueSuffix; ?>-hidden" type="hidden" name="mphb_check_out_date" value="<?php echo esc_attr( $checkOutDate ); ?>" />
		</p>
		<?php // @todo Maybe it was better to use BookingFormShortcode ([mphb_availability])? ?>
		<?php if ( !$isDirectBooking ) { ?>
			<p class="mphb-adults-wrapper">
				...
				<br />
				<select id="mphb_adults-<?php echo $uniqueSuffix; ?>" name="mphb_adults">
			...
			<p class="mphb-check-children-date-wrapper">
				...
				<br />
				<select id="mphb_children-<?php echo $uniqueSuffix; ?>" name="mphb_children">
		...
		<?php if ( $isDirectBooking ) { ?>
			<div class="mphb-reserve-room-section mphb-hide">
				<p class="mphb-rooms-quantity-wrapper">
					<select class="mphb-rooms-quantity" id="mphb-rooms-quantity-<?php echo $typeId; ?>" name="mphb_rooms_details[<?php echo $typeId; ?>]">
						<option value="1" selected="selected">1</option>
					</select>
					<span class="mphb-available-rooms-count"><?php
						echo sprintf( __( 'of %d accommodation available.', 'motopress-hotel-booking' ), 1 );
						?></span>
				</p>
